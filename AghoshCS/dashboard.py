...
def open_donor_mgmt():
    ...
    donor_ID = tk.Entry(form_frame, font=FONTS["entry"], width=30, state="readonly", readonlybackground="#f1f5f9")
    ...
    #collector_dropdown.bind("<<ComboboxSelected>>", generate_next_office_donor_id)

    def generate_next_office_donor_id(event=None):
        """
        Generate next Office Donor ID based on selected collector.

        Format:
            Donor-AGH-001-002

        Where:
            AGH = fixed organization/program code
            001 = collector code
            002 = next donor number for that collector
        """

        collector_name = collector_var.get().strip()

        if not collector_name:
            return

        if collector_name not in collector_id_map:
            return

        collector_id = collector_id_map[collector_name]

        # Collector code as 3 digits
        collector_code = str(collector_id).zfill(3)

        prefix = f"Donor-AGH-{collector_code}-"
        like_pattern = prefix + "%"
        ...
        try:
            ...
            cursor.execute("""
                SELECT IFNULL(MAX(CAST(SUBSTR(OfficeDonorID, -3) AS INTEGER)), 0)
                FROM tblDonors
                WHERE OfficeDonorID LIKE ?
            """, (like_pattern,))

            last_no = cursor.fetchone()[0] or 0
            next_no = int(last_no) + 1

            next_office_id = f"{prefix}{next_no:03d}"

            donor_ID.config(state="normal")
            donor_ID.delete(0, tk.END)
            donor_ID.insert(0, next_office_id)
            donor_ID.config(state="readonly")

        except Exception as e:
            messagebox.showerror(
                "Database Error",
                f"Failed to generate Office Donor ID:\n{e}"
            )
    ...
    def clear_form():
       ...
       # donor_contact.delete(0, tk.END)
        ...
        if collector_dropdown["values"]:
            collector_dropdown.current(0)
            generate_next_office_donor_id()
        else:
            donor_ID.delete(0, tk.END)
            donor_ID.insert(0, "Donor-___-___-___")
...
def open_sponsorship_mgmt():
    ...
    selected_old_amount  = tk.DoubleVar(value=0.0)
    ...
    form_canvas.configure(
        yscrollcommand=form_v_scroll.set,
    ...
    def _on_canvas_configure(e):
        form_canvas.itemconfig(inner_id, width=max(e.width, inner.winfo_reqwidth()))
    ...
    remaining_committed_entry = tk.Entry(
        amt_lf,
        font=FONT_ENTRY,
        width=18,
        relief="solid",
    ...
    start_date = DateEntry(date_lf, date_pattern="dd/MM/yyyy", font=FONT_ENTRY, width=14)
    start_date.grid(row=0, column=1, sticky="ew", **PAD)

    lbl(date_lf, "End Date:", 1, 0)
    end_date = DateEntry(date_lf, date_pattern="dd/MM/yyyy", font=FONT_ENTRY, width=14)
    ...
    # TREEVIEW
    # ─────────────────────────────────────────────────────────────────────────
    tree_lf = tk.LabelFrame(
        body,
        text="  Sponsorship Records  ",
        font=FONT_SECTION,
        fg=ACCENT,
        bg=FRAME_BG,
        bd=1,
    ...
    def _parse_db_date(raw):
        ...
        try:
            return _dt.datetime.strptime(str(raw)[:10], "%Y-%m-%d")
    ...
    def load_all_children_for_edit():
        nonlocal children_dict
    ...
    def _write_child_block(ws, child, start_excel_row, sno,
        ...
        for row_offset in range(total_rows):
            ...
            for col_offset, val in enumerate(child_vals):
                ...
                c.font = data_font
                c.fill = fill
                c.alignment = c_align_top if col_num in (1, 2, 4) else l_align_top
                c.border = _border(
        ...
        if total_rows > 1:
            ...
            for col_num in range(1, N_CHILD_COLS + 1):
                ...
                mc.font = data_font
                mc.fill = fill
                mc.alignment = c_align_top if col_num in (1, 2, 4) else l_align_top
                mc.border = _border(
                    left=medium if col_num == 1 else thin,
                    right=medium if col_num == N_CHILD_COLS else thin,
                    top=medium,
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # EXCEL EXPORT 1 — Sponsored children only
    # ─────────────────────────────────────────────────────────────────────────
    def export_to_excel():
        import datetime as _dt
        from tkinter import filedialog
        ...
        blank_fill = PatternFill("solid", start_color="FFF9F0")
    ...
    def export_all_children_excel():
        ...
        filepath = filedialog.asksaveasfilename(
            ...
            filetypes=[("Excel Files", "*.xlsx")],
            initialfile=default_name,
            title="Save All Children Export"
        )
        ...
        fills_un = [
            PatternFill("solid", start_color="FFFFF0"),
    ...
    def on_donor_selected(event=None):
        ...
        try:
            ...
            if selected_id.get() != "":
                old_amount = float(selected_old_amount.get() or 0)
                remaining_monthly = total_monthly - total_allocated + old_amount
            else:
    ...
    def on_tree_select(event=None):
        try:
            ...
            cursor.execute("""
                ...
                FROM tblSponsorships s
                JOIN tblDonors d ON s.DonorID = d.DonorID
                WHERE s.SponsorshipID = ?
            """, (sponsorship_id,))

            row = cursor.fetchone()

            if not row:
                messagebox.showerror("Error", "Selected sponsorship record not found.")
                return

            sponsorship_id = row.SponsorshipID
            child_id = row.ChildID
            donor_id = row.DonorID
            amount = float(row.SponsorshipAmount or 0)
            ...
            selected_id.set(str(sponsorship_id))
            selected_old_amount.set(amount)

            # Load all children during edit mode.
            # Reason: selected child may already be fully sponsored,
            # so normal load_children() may not contain it.
            load_all_children_for_edit()

            for text, cid in children_dict.items():
                if str(cid) == str(child_id):
                    child_combo.set(text)
                    break

            # Set collector.
            for text, cid in collector_lookup.items():
                if str(cid) == str(collector_id):
                    collector_combo.set(text)
                    break

            # Load all donors for this collector during edit mode.
            # Reason: current donor may be fully allocated, so normal donor loader may hide it.
            donor_dict.clear()

            if collector_id:
                cursor.execute("""
                    SELECT DonorID, FullName
                    FROM tblDonors
                    WHERE IsActive = 1
                      AND CollectorID = ?
                    ORDER BY FullName
            ...
            for r in donor_rows:
                donor_dict[f"{r.FullName} (ID:{r.DonorID})"] = r.DonorID

            donor_combo.set_completion_list(list(donor_dict.keys()))

            for text, did in donor_dict.items():
                if str(did) == str(donor_id):
                    donor_combo.set(text)
                    break
            ...
            required = float(req_row[0] or 0) if req_row else 0
            required_amount_var.set(f"{required:.2f}")

            # Still required without old amount.
            remaining_without_old = get_remaining_amount_for_child(child_id)
            still_required_amount_var.set(f"{remaining_without_old:.2f}")

            # Donor remaining for edit.
            cursor.execute("""
                SELECT IFNULL(SUM(MonthlyAmount), 0)
                FROM tblCommitments
                WHERE DonorID = ?
            ...
            remaining_for_edit = total_monthly - total_allocated + amount

            remaining_committed_entry.config(state="normal")
            remaining_committed_entry.delete(0, tk.END)
            remaining_committed_entry.insert(0, f"{remaining_for_edit:.0f}")
            remaining_committed_entry.config(state="readonly")
            ...
            donor_summary_label.config(
                text=(
                    f"Monthly Commitment :  Rs. {total_monthly:,.0f}\n"
                    f"Allocated Sponsorships :  Rs. {total_allocated:,.0f}\n"
                    f"Available for Editing :  Rs. {remaining_for_edit:,.0f}"
                ),
                fg=ACCENT
            )

            # Dates.
            start_dt = _parse_db_date(start)
            end_dt = _parse_db_date(end)

            if start_dt:
                try:
                    start_date.set_date(start_dt)
                except Exception:
                    pass

            if end_dt:
                try:
                    end_date.set_date(end_dt)
                except Exception:
                    pass
            ...
            messagebox.showerror(
                "Error",
                f"Failed to load sponsorship for editing:\n{e}"
            )
    ...
    def assign_sponsorship():
        try:
            ...
            if is_edit_mode:
                old_amount = float(selected_old_amount.get() or 0)
                allowed_child_amount = still_required + old_amount
            else:
                allowed_child_amount = still_required

            if amount > allowed_child_amount:
                messagebox.showwarning(
                    "Validation Error",
                    f"Sponsorship amount (Rs. {amount:,.2f}) cannot exceed "
                    f"the allowed child amount (Rs. {allowed_child_amount:,.2f})."
                )
                return

            remaining_donor_amount = float(remaining_committed_entry.get() or 0)

            if amount > remaining_donor_amount:
                messagebox.showwarning(
                    "Validation Error",
                    f"Sponsorship amount (Rs. {amount:,.2f}) cannot exceed "
                    f"the donor's available commitment amount "
                    f"(Rs. {remaining_donor_amount:,.2f})."
                )
                return
            ...
            if is_edit_mode:
                messagebox.showinfo("Success", "Sponsorship updated successfully!")
            else:
                messagebox.showinfo("Success", "Sponsorship assigned successfully!")
    ...
    def del_sponsorship():
        ...
        if not selected_item:
            messagebox.showwarning("No Selection", "Please select a sponsorship row first.")
            return
        ...
        if not sponsorship_id:
            messagebox.showwarning("No Selection", "Invalid sponsorship selection.")
            return

        if not messagebox.askyesno(
            "Confirm Delete",
            f"Delete this sponsorship?\n\n"
            f"Donor: {donor_name}\n"
            f"Amount: Rs. {amount}\n"
    ...
    assign_btn = tk.Button(
        btn_lf,
        text="✔  Assign Sponsorship",
        command=assign_sponsorship,
        state="disabled",
        font=FONT_BTN,
        bg=BTN_GREEN,
        fg="white",
    ...
    tk.Button(
        btn_lf,
        text="🗑  Delete Sponsorship",
        command=del_sponsorship,
        font=FONT_BTN,
        bg=BTN_RED,
        fg="white",
        relief="flat",
    ...
    tk.Button(
        btn_lf,
        text="📋  Export All Children",
        command=export_all_children_excel,
        font=FONT_BTN,
        bg=BTN_PURPLE,
        fg="white",
        relief="flat",
    ...
    win.bind('<Return>', lambda e: (
        assign_btn.invoke()
        if assign_btn.winfo_ismapped()
        and str(assign_btn.cget("state")) != "disabled"
        else update_btn.invoke()
        if update_btn.winfo_ismapped()
        and str(update_btn.cget("state")) != "disabled"
        else None
    ))
...
#no need of documents section here
        ...
        def resolve_image_path(path):
            """
            Safely resolve image/document path for Windows Server / LAN shortcut usage.
            Works with:
            - absolute paths
            - relative paths
            - paths stored as photo\\file.jpg
            - paths stored as only file.jpg
            """
            ...
            # If database already has absolute path
            ...
            possible_paths = [
                ...
                os.path.join(base_dir, "photo", path),
                os.path.join(os.getcwd(), path),
                os.path.join(os.getcwd(), "photo", path),
            ]
...
# ============================================================
        def generate_clean_pdf():
            try:
                ...
                # CHILD PHOTO
                # --------------------------------------------------
                image_path = resolve_image_path(child_data.get("PhotoPath", ""))
                ...
                if image_path:
                    try:
                        ...
                        print(f"Child photo error: {image_path} -> {e}")
                else:
                    print(f"Child photo not found: {child_data.get('PhotoPath', '')}")
                ...
                field_labels = {
                    ...
                    "MotherDeathDate": "Mother's Death Date",
                    "MotherCNIC": "Mother's CNIC",
                ...
                # Dynamic sibling / meet person labels
                for i in range(1, 6):
                    field_labels[f"Sibling{i}Name"] = f"Sibling {i} Name"
                    field_labels[f"Sibling{i}Gender"] = f"Sibling {i} Gender"
                ...
                def draw_clean_section(title, fields):
                    ...
                    for f in fields:
                        ...
                        # Special rule: skip MotherDeathDate if mother is alive
                ...
                doc_fields = [
                    ...
                    ("Father Death Certificate", child_data.get("DocFatherDeathCert", "")),
                    ("School Certificate", child_data.get("DocSchoolCertificate", "")),
                    ("Other Document 1", child_data.get("OtherDoc1", "")),
                    ("Other Document 2", child_data.get("OtherDoc2", "")),
                    ("Other Document 3", child_data.get("OtherDoc3", "")),
                ]
                ...
                for label, path in doc_fields:
                    ...
                    if full_path:
                        ...
                        try:
                            ...
                            print(f"Document image error [{label}]: {full_path} -> {e}")

                            pdf.setFont("Helvetica", 9)
                            pdf.drawString(x_margin, y, f"{label}: Could not load image")
                            y -= 20

                    else:
                        if str(path).strip() not in ["", "None", "null"]:
                            print(f"Document not found [{label}]: {path}")

                if not found_any_document:
                    pdf.setFont("Helvetica", 11)
                    pdf.drawString(x_margin, y, "No attached document images found.")
